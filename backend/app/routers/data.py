...
from app.models import DeviceSnapshot, Reading, User
...
async def time_series(
    ...
    if bucket == "minute" or len(rows) < 50:
...
async def summary(
    ...
    return sanitize_for_json(
        {
            "today_pv_kwh": round(delta(first.pv_kwh_total, last.pv_kwh_total), 2),
            "today_net_kwh": round(delta(first.net_kwh_total, last.net_kwh_total), 2),
            "today_load_kwh": round(delta(first.load_kwh_total, last.load_kwh_total), 2),
